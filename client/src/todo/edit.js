import React, {useEffect, useState} from 'react'
import jsonFetch from 'lib/json-fetch'

const getInitialState = () => ({
    description: '',
    responsible: '',
    priority:'',
    completed: false
  })
  
export default ({match: {params: {id}}, history}) => {
  const [{
          description,
          responsible,
          priority,
          completed
        }, setTodo] = useState(getInitialState()),

        handleInputChange = ({target: {name, type, checked, value}}) => setTodo(todo => ({
          ...todo,
          [name]: (type === 'checkbox' ? checked : value)
        })),

        handleSubmit = async (e) => {
          e.preventDefault()
          try {
            await jsonFetch(`http://localhost:4000/api/todos/${id}`, {
              method: 'PUT',
              body: JSON.stringify({
                description,
                responsible,
                priority,
                completed
              })
            })
            history.push('/');
          }
          catch(e) {
            alert(e)
          }
        }

  useEffect(() => {
    (async () => {
      try {
        setTodo(await jsonFetch(`http://localhost:4000/api/todos/${id}`))
      }
      catch(e) {
        alert(e)
      }
    })()
  }, [id])

  return (
    <div style={{marginTop: 10}}>
      <h3>Update Todo</h3>
      <form onSubmit={handleSubmit}>
        <div className="form-group"> 
          <label htmlFor="description">Description: </label>
          <input  type="text"
                  name="description"
                  id="description"
                  className="form-control"
                  value={description}
                  disabled={completed}
                  onChange={handleInputChange} />
        </div>
        <div className="form-group">
          <label htmlFor="responsible">Responsible: </label>
          <input  type="text" 
                  name="responsible"
                  id="responsible"
                  className="form-control"
                  value={responsible}
                  disabled={completed}
                  onChange={handleInputChange} />
        </div>
        <div className="form-group">
          <div className="form-check form-check-inline">
              <input  className="form-check-input" 
                      type="radio" 
                      name="priority" 
                      id="priorityLow" 
                      value="Low"
                      checked={priority === 'Low'} 
                      disabled={completed}
                      onChange={handleInputChange} />
              <label className="form-check-label" htmlFor="priorityLow">Low</label>
          </div>
          <div className="form-check form-check-inline">
            <input  className="form-check-input" 
                    type="radio" 
                    name="priority" 
                    id="priorityMedium" 
                    value="Medium" 
                    checked={priority === 'Medium'} 
                    disabled={completed}
                    onChange={handleInputChange} />
            <label className="form-check-label" htmlFor="priorityMedium">Medium</label>
          </div>
          <div className="form-check form-check-inline">
            <input  className="form-check-input" 
                    type="radio" 
                    name="priority" 
                    id="priorityHigh" 
                    value="High" 
                    checked={priority === 'High'} 
                    disabled={completed}
                    onChange={handleInputChange} />
            <label className="form-check-label" htmlFor="priorityHigh">High</label>
          </div>
          <div className="form-check">
            <input  className="form-check-input"
                    type="checkbox"
                    name="completed"
                    id="completed"
                    checked={completed}
                    value={completed}
                    onChange={handleInputChange} />
            <label className="form-check-label" htmlFor="completed">Completed</label>                        
          </div>

        </div>

        <div className="form-group">
          <input type="submit" value="Update Todo" className="btn btn-primary" />
        </div>
      </form>
    </div>
  )
}
